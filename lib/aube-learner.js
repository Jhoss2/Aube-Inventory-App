...
import { llmLocalPret, sessionApprentissageMutuel } from './aube-local-llm';
...
var GEMINI_URL_BASE = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=';
...
export async function lancerSessionApprentissage(appData, onProgres) {
  ...
  // Si pas de clé Gemini MAIS LLM local disponible → apprentissage via LLM local
  if (!getApiKey()) {
    if (llmLocalPret()) {
      if (onProgres) onProgres({ statut: 'debut', message: 'Apprentissage via le cerveau local (LLM)...' });
      var lacunes = await getLacunes(10);
      var apprisLocal = 0;

      for (var li = 0; li < lacunes.length; li++) {
        if (onProgres) onProgres({
          statut: 'apprentissage',
          message: 'LLM enseigne à Aube : ' + lacunes[li].question.slice(0, 40) + '...',
          progression: Math.round(((li + 1) / lacunes.length) * 100),
        });

        var repLLM = await sessionApprentissageMutuel(
          lacunes[li].question,
          appData,
          async function(q, r) {
            await sauvegarderReponseGemini(q, r);
          }
        );
        ...
        if (repLLM) {
          await _db.runAsync(
            'INSERT INTO connaissances (question, variantes, reponse, mots_cles, explication, timestamp) VALUES (?, ?, ?, ?, ?, ?)',
            [lacunes[li].question, '', repLLM, '', 'Enseigné par LLM local', new Date().toISOString()]
          );
          await marquerApprise(lacunes[li].id);
          apprisLocal++;
        }
      }
      ...
      if (onProgres) onProgres({
        statut: 'termine',
        message: '✅ Le LLM a enseigné ' + apprisLocal + ' leçon(s) à Aube !',
        nbApprises: apprisLocal,
      });
      return { succes: true, nbApprises: apprisLocal, source: 'llm_local' };
    }

    if (onProgres) onProgres({ statut: 'erreur', message: 'Ni connexion ni LLM local disponible.' });
    return { succes: false };
  }
