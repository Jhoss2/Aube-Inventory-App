...
export function detecterIntentionAction(texte) {
  ...
  // Notification créateur — priorité haute
  if (contient(t, ['belem', 'createur', 'ebenezer', 'monsieur belem'])) return 'NOTIF_CREATEUR';
  if (contient(t, ['notifie', 'envoie un message', 'invite']) && contient(t, ['monsieur', 'belem', 'createur'])) return 'NOTIF_CREATEUR';

  // PDF / Téléchargement
  if (contient(t, ['telecharge', 'pdf', 'genere le pdf', 'exporte', 'imprimer'])) {
    if (contient(t, ['rapport', 'bilan', 'inventaire', 'general'])) return 'RAPPORT_PDF';
    if (contient(t, ['salle', 'classe', 'bureau', 'local', 'contenu'])) return 'PDF_SALLES';
    return 'PDF_SALLES';
  }

  // Rapport
  if (contient(t, ['rapport', 'bilan', 'synthese', 'redige', 'etat general', 'analyse'])) {
    if (contient(t, ['telecharge', 'pdf', 'genere', 'exporte'])) return 'RAPPORT_PDF';
    return 'RAPPORT_TEXTE';
  }

  // Matériels — avant salles pour éviter confusion
  if (contient(t, ['nouveau materiel', 'ajoute un materiel', 'cree un materiel', 'ajouter un materiel', 'nouveau equipement'])) return 'CREATE_MATERIEL';
  if (contient(t, ['modifie le materiel', 'change l etat', 'mets a jour le materiel', 'actualise le materiel'])) return 'UPDATE_MATERIEL';
  if (contient(t, ['supprime le materiel', 'efface le materiel', 'retire le materiel'])) return 'DELETE_MATERIEL';

  // Salles — détection large
  if (contient(t, ['cree', 'ajoute', 'nouvelle', 'creer', 'ajouter']) &&
      contient(t, ['salle', 'classe', 'bureau', 'local', 'amphi', 'amphitheatre'])) return 'CREATE_ROOM';
  if (contient(t, ['supprime', 'efface', 'retire', 'enleve']) &&
      contient(t, ['salle', 'classe', 'bureau', 'local'])) return 'DELETE_ROOM';
  if (contient(t, ['modifie', 'renomme', 'change le nom', 'modifier']) &&
      contient(t, ['salle', 'classe', 'bureau'])) return 'UPDATE_ROOM';

  // Stats
  if (contient(t, ['statistique', 'stat', 'combien', 'total', 'resume', 'vue globale'])) return 'STATS';
...
export async function executerActionComplete(intention, texte, appData, appContext) {
  ...
  if (intention === 'CREATE_ROOM') {
    ...
    // Extraction multiple — chercher toutes les salles mentionnées
    var blocId = extraireBloc(texte);
    if (!blocId) return 'Précisez le bloc. Exemple : "dans le bloc B"';

    // Chercher tous les patterns "nommée X", "nom : X", "salle X", "appelée X"
    var nomsTrouves = [];

    // Pattern 1 : "nommée X et une nommée Y" ou "nommée X et nommée Y"
    var matchNommee = texte.match(/nomm[eé]e?\s+"?([^",\net]+)"?/gi) || [];
    for (var ni = 0; ni < matchNommee.length; ni++) {
      var n = matchNommee[ni].replace(/nomm[eé]e?\s+/i, '').replace(/"/g, '').trim();
      if (n && n.length > 1) nomsTrouves.push(n);
    }

    // Pattern 2 : "nom : X" ou "noms : X, Y"
    if (nomsTrouves.length === 0) {
      var matchNom = texte.match(/nom\s*:\s*([^\n.]+)/i);
      if (matchNom && matchNom[1]) {
        var parts = matchNom[1].split(/\s+et\s+|\s*,\s*/i);
        for (var pi = 0; pi < parts.length; pi++) {
          var p = parts[pi].trim();
          if (p.length > 1) nomsTrouves.push(p);
        }
      }
    }

    // Pattern 3 : "salle XX" directement dans le texte
    if (nomsTrouves.length === 0) {
      var matchSalles = texte.match(/(?:salle|classe|amphi|bureau)\s+[\w\d./-]+/gi) || [];
      for (var si2 = 0; si2 < matchSalles.length; si2++) {
        nomsTrouves.push(matchSalles[si2].trim());
      }
    }

    if (nomsTrouves.length === 0) return 'Précisez le(s) nom(s) de la salle. Exemple : "nommée Salle 06"';

    // Créer toutes les salles trouvées
    var creees = [];
    for (var ci = 0; ci < nomsTrouves.length; ci++) {
      appContext.addSalle({
        id: 'salle-' + Date.now() + '-' + ci,
        name: nomsTrouves[ci],
        blockId: blocId,
        level: 'Niveau 1',
        image: null,
      ...
      creees.push('"' + nomsTrouves[ci] + '"');
    }

    return '✅ ' + creees.length + ' salle(s) créée(s) dans le Bloc ' + blocId + ' : ' + creees.join(', ') + ' !';
  }
