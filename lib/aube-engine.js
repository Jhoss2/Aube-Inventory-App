...
export async function chatWithAubeStream(userText, systemPrompt, appData, history, onChunk, session, appContext) {
  ...
  if (repRaisonnement) {
    ...
    // Si cle dispo, enrichir avec Gemini en parallele (pas d'attente)
    if (apiKey) {
      _appelGeminiEnrichissement(userText, systemPrompt, appData, history, sess, isEB1).catch(function() {});
    }
    return;
  }
