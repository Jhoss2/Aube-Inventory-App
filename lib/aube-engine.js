...
export async function chatWithAubeStream(userText, systemPrompt, appData, history, onChunk, session, appContext) {
  ...
  if (!GEMINI_API_KEY) {
    var repOff = repondreOffline(userText, appData) || 'Je suis hors ligne. Configure la cle API pour les reponses IA.';
    await saveMessage(sess, 'user', userText);
    onChunk(repOff);
  ...
  try {
    ...
    if (!response.ok) {
      var fallback = repondreOffline(userText, appData) || 'Erreur reseau. Mode offline : ' + (repondreOffline(userText, appData) || 'Verifie ta connexion.');
      onChunk(fallback);
    ...
    var rep2 = repondreOffline(userText, appData) || 'Hors ligne. Verifie ta connexion.';
    onChunk(rep2);
