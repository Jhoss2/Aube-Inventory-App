...
function detecterAction(texte) {
  ...
  // Creation de salle
  if ((t.indexOf('cree') !== -1 || t.indexOf('ajoute') !== -1 || t.indexOf('nouvelle salle') !== -1 || t.indexOf('creer une salle') !== -1 || t.indexOf('cree une salle') !== -1) &&
      (t.indexOf('salle') !== -1 || t.indexOf('classe') !== -1 || t.indexOf('bureau') !== -1)) return 'CREATE_ROOM';

  // Suppression de salle
  if ((t.indexOf('supprime') !== -1 || t.indexOf('efface') !== -1 || t.indexOf('retire') !== -1) &&
      (t.indexOf('salle') !== -1 || t.indexOf('classe') !== -1)) return 'DELETE_ROOM';
...
export async function chatWithAubeStream(userText, systemPrompt, appData, history, onChunk, session, appContext) {
  ...
  // Partager la cle avec le moteur d'embeddings
  ...
  // ── Actions directes simples (offline) ──
  ...
  // ── Actions complètes — Aube manipule son biotope ──
  ...
  // ── COUCHE 2 : Tenter Gemini si cle disponible ──
  if (apiKey) {
    await saveMessage(sess, 'user', userText);
    try {
  ...
  // ── COUCHE 3 : Connaissances apprises + cache + general (offline) ──
  ...
  if (repEmbed) {
    await saveMessage(sess, 'user', userText);
    onChunk(repEmbed.reponse);
    await saveMessage(sess, 'model', repEmbed.reponse);
    return;
  }
  ...
  var repFinal = repGeneral || 'Je suis momentanément sans connexion. Je reste disponible pour les questions sur l\'inventaire.';
