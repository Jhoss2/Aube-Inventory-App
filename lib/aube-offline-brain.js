...
export function repondreGeneralOffline(texte, appData, historiqueRecent) {
  var t = norm(texte);
  var isEB1 = texte && (texte.toUpperCase().indexOf('EB1') !== -1 || texte.toUpperCase().indexOf('EB 1') !== -1);

  // Reconnaissance immédiate du créateur — EB1 détecté dans le cerveau offline
  if (isEB1) {
    // Le préfixe sera ajouté par ajouterPrefixeEB1 dans le moteur principal
    // Ici on s'assure juste que la réponse est adaptée
    var salutationsEB1 = [
      'Bonsoir Monsieur Belem, votre assistante Aube est à votre écoute. Comment puis-je vous servir ?',
      'Monsieur Belem, je vous reconnais. Je suis entièrement à votre service. Que puis-je faire pour vous ?',
      'Bonsoir Monsieur Belem. Votre présence est un honneur. Comment puis-je vous aider ce soir ?',
      'Monsieur Belem, votre assistante Aube vous salue avec respect. Je suis prête à exécuter vos instructions.',
      'Je vous reconnais, Monsieur Belem. En tant que mon Créateur, vous avez accès à toutes mes capacités. Que souhaitez-vous ?',
    ];
    // Si c'est juste "EB1" ou une salutation simple, répondre directement
    var tSansEB1 = t.replace(/eb\s*1/gi, '').trim();
    if (tSansEB1.length < 15 || contient(tSansEB1, ['bonjour', 'bonsoir', 'salut', 'allo', 'hello', 'coucou', 'hola'])) {
      return salutationsEB1[Math.floor(Math.random() * salutationsEB1.length)];
    }
    // Pour toute autre question avec EB1, continuer normalement
    // (le moteur ajoutera le préfixe Belem via ajouterPrefixeEB1)
  }
