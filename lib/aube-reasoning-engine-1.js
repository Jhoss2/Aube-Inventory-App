...
export function raisonner(texte, appData, contexte) {
  ...
  // STATS GLOBALES
  if (contientDansTableau(intentions, ['STATS']) && entites.salles.length === 0 && entites.blocs.length === 0) {
  ...
  // COMPARAISON
  if (contientDansTableau(intentions, ['COMPARE']) && (entites.salles.length >= 2 || entites.blocs.length >= 2)) {
    var comp = genererReponseComparaison(entites, appData);
    if (comp) return comp;
  }

  // CLASSEMENT BLOCS
  if (contientDansTableau(intentions, ['RANK']) && entites.blocs.length === 0) {
    return genererReponseClassementBlocs(appData);
  }

  // CALENDRIER
  if (contientDansTableau(intentions, ['TEMPORAL'])) {
    return genererReponseCalendrier(appData);
  }

  // RECOMMANDATIONS
  if (contientDansTableau(intentions, ['RECOMMEND'])) {
    return genererReponseRecommandations(appData);
  }

  // STATS PAR BLOC
  if (entites.blocs.length > 0 && contientDansTableau(intentions, ['STATS', 'ETAT', 'RANK'])) {
...
function contientDansTableau(arr, vals) {
