...
export default function HomeScreen() {
  ...
  const aubeAvatar      = (appData && appData.settings && appData.settings.assistantAvatar) || null;
  ...
  const handleSearch = (query: string) => {
    ...
    for (var i = 0; i < salles.length; i++) {
      if ((salles[i].name || '').toLowerCase().indexOf(q) !== -1) {
        results.push({ type: 'salle', label: salles[i].name, sub: 'Bloc ' + (salles[i].blockId || '?'), id: salles[i].id });
      }
    }
    for (var j = 0; j < materiels.length; j++) {
      var m = materiels[j];
      if ((m.nom || '').toLowerCase().indexOf(q) !== -1 || (m.category || '').toLowerCase().indexOf(q) !== -1) {
        var sal = salles.find((s: any) => String(s.id) === String(m.roomId));
        results.push({ type: 'materiel', label: m.nom, sub: (m.category || '') + (sal ? ' — ' + sal.name : ''), roomId: m.roomId, roomName: sal ? sal.name : '' });
      }
    }
        ...
        >
          ...
          <View style={styles.searchContainer}>
            ...
            <TouchableOpacity style={styles.botButton} onPress={() => router.push('/chat-aube')}>
              {aubeAvatar
                ? <Image source={{ uri: aubeAvatar }} style={styles.botAvatar} />
          ...
          {searchResults.length > 0 && (
            <View style={[styles.searchResultsBox, styles.glow]}>
              {searchResults.map((r: any, idx: number) => (
                ...
                >
                  <MaterialCommunityIcons
                    name={r.type === 'salle' ? 'door' : 'package-variant'}
                    size={16} color="#1A237E" style={{ marginRight: 8 }}
                  />
                  <View>
                    <Text style={[styles.resultLabel, styles.boldSerif]}>{r.label}</Text>
                    <Text style={[styles.resultSub, styles.boldSerif]}>{r.sub}</Text>
                  </View>
                </TouchableOpacity>
              ))}
            </View>
...
const styles = StyleSheet.create({
  ...
  botAvatar: { width: 28, height: 28, borderRadius: 14 },
