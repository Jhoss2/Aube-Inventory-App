...
import { apprendreDepuisUrl, statsMemoire } from '@/lib/aube-memory';
...
export default function SettingsScreen() {
  ...
  const [urlApprentissage, setUrlApprentissage] = useState('');
  const [apprenantWeb, setApprenantWeb] = useState(false);
  const [webStatus, setWebStatus] = useState('');
  const [memoireStats, setMemoireStats] = useState<any>(null);

  useEffect(() => {
    statsMemoire().then(function(s) { setMemoireStats(s); }).catch(function() {});
  }, []);

  const handleApprendreUrl = async () => {
    if (!urlApprentissage.trim()) { Alert.alert('Erreur', 'Entrez une URL valide.'); return; }
    var url = urlApprentissage.trim();
    if (!url.startsWith('http')) url = 'https://' + url;
    setApprenantWeb(true);
    setWebStatus('Téléchargement en cours...');
    var r = await apprendreDepuisUrl(url);
    setApprenantWeb(false);
    if (r.succes) {
      setWebStatus('✅ ' + r.stockes + ' sections apprises depuis ' + r.source);
      setUrlApprentissage('');
      var s = await statsMemoire();
      setMemoireStats(s);
    } else {
      setWebStatus('❌ ' + (r.message || 'Erreur'));
    }
  };
  ...
  const [openSections, setOpenSections] = useState({
    ...
    blocs: false, ecrans: false, pdf: false, affiches: false, chat: false, ia: false,
  });
  ...
  const handleLogin = () => {
    ...
    else { Alert.alert("Accès refusé", "Mot de passe incorrect."); }
  };
  ...
  const pickImage = async (field: string) => {
    ...
    if (!result.canceled) updateSettings({ [field]: result.assets[0].uri });
  };

  const SettingRow = ({ label, field }: { label: string, field: string }) => {
    const s = (appData.settings || {}) as any;
    const hasValue = !!s[field];
    return (
            ...
            <View style={styles.authModalContent}>
              ...
              <View style={styles.haloWrapper}>
                <TextInput 
                  ...
                  secureTextEntry value={password} onChangeText={setPassword} autoFocus
                />
              </View>
  ...
  // ── Écran paramètres ──────────────────────────────────────────────────────
  const settingsBg = (appData && appData.settings && appData.settings.settingsBg) || null;
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: 'white' }}>
      {settingsBg && (
        <ImageBackground source={{ uri: settingsBg }} style={StyleSheet.absoluteFill} resizeMode="cover" />
      )}
      ...
      <ScrollView style={{ flex: 1 }}>
        ...
        {openSections.security && (
          <View style={styles.accordionContent}>
            ...
            <View style={{ marginTop: 15 }}>
              <Text style={[styles.rowLabel, styles.boldSerif]}>
                Intensité du flou : {Math.round(((appData && appData.settings && appData.settings.authBlur) || 0))}%
              </Text>
              <Slider
                ...
                minimumTrackTintColor="#FF0000" maximumTrackTintColor="#ddd" thumbTintColor="#1D3583"
              />
            </View>
          </View>
        )}
        ...
        {/* Blocs — avec arrière-plan de l'écran bloc */}
        ...
        {openSections.blocs && (
          <View style={styles.accordionContent}>
            <SettingRow label="Arrière-plan Écran Bloc Details (commun à A, B, C...)" field="blocDetailsBg" />
            {['A', 'B', 'C', 'D', 'E', 'F'].map((b) => (
        ...
        {/* Arrière-plans des autres écrans (chacun indépendant, pas de regroupement) */}
        <TouchableOpacity style={styles.accordionHeader} onPress={() => toggleSection('ecrans')}>
          <Text style={[styles.accordionTitle, styles.boldSerif]}>Arrière-plans des Écrans</Text>
          <Feather name={openSections.ecrans ? "chevron-up" : "chevron-down"} size={20} color="#8B1A1A" />
        </TouchableOpacity>
        {openSections.ecrans && (
          <View style={styles.accordionContent}>
            <SettingRow label="Contenu de Salle (liste + détail matériel)" field="roomContentsBg" />
            <SettingRow label="Ajout de Salle"         field="addRoomBg" />
            <SettingRow label="Détails de la Salle"  field="roomDetailsBg" />
            <SettingRow label="Éditeur de Note"       field="noteEditorBg" />
            <SettingRow label="Ajout de Matériel"     field="addMaterialBg" />
            <SettingRow label="Alertes"               field="alertsBg" />
            <SettingRow label="Paramètres"            field="settingsBg" />
            <SettingRow label="Catégories"            field="categoriesBg" />
          </View>
        )}

        {/* Document PDF */}
        <TouchableOpacity style={styles.accordionHeader} onPress={() => toggleSection('pdf')}>
          <Text style={[styles.accordionTitle, styles.boldSerif]}>Document PDF</Text>
          <Feather name={openSections.pdf ? "chevron-up" : "chevron-down"} size={20} color="#8B1A1A" />
        </TouchableOpacity>
        {openSections.pdf && (
          <View style={styles.accordionContent}>
            <SettingRow label="Logo (en haut à gauche du PDF)" field="pdfLogo" />
          </View>
        )}
        ...
        {openSections.ia && (
          <View style={styles.accordionContent}>
            ...
            {/* Mémoire stats */}
            {memoireStats && (
              <View style={{ marginTop: 16, padding: 12, backgroundColor: '#f0f4ff', borderRadius: 12 }}>
                <Text style={[styles.boldSerif, { fontSize: 13, color: '#1D3583', marginBottom: 6 }]}>
                  🧠 Mémoire d'Aube
                </Text>
                <Text style={[styles.boldSerif, { fontSize: 12, color: '#555' }]}>
                  {'Préférences : ' + (memoireStats.profil || 0) + ' · Faits web : ' + (memoireStats.faits || 0) + ' · Corrections : ' + (memoireStats.corrections || 0)}
                </Text>
              </View>
            )}
            ...
            <View style={{ marginTop: 16 }}>
              <Text style={[styles.boldSerif, { fontSize: 13, color: '#555', marginBottom: 8 }]}>
                🌐 Apprendre depuis une URL
              </Text>
              <View style={{ flexDirection: 'row', gap: 8, alignItems: 'center' }}>
                <TextInput
                  value={urlApprentissage}
                  onChangeText={setUrlApprentissage}
                ...
                />
                <TouchableOpacity
                  onPress={handleApprendreUrl}
                  disabled={apprenantWeb}
                  style={[styles.saveKeyBtn, apprenantWeb && { opacity: 0.5 }]}
                >
                  <Feather name={apprenantWeb ? "loader" : "download"} size={20} color="white" />
                </TouchableOpacity>
              </View>
              {webStatus !== '' && (
                <View style={[styles.statusBox, { marginTop: 8 }]}>
                  <Text style={[styles.boldSerif, {
                    fontSize: 12,
                    color: webStatus.startsWith('✅') ? '#059669' : '#dc2626',
                  }]}>{webStatus}</Text>
                </View>
              )}
            </View>
...
const styles = StyleSheet.create({
  ...
  haloWrapper:      { width: '100%', shadowColor: "#FF0000", shadowOffset: { width: 0, height: 0 }, shadowOpacity: 1, shadowRadius: 15, elevation: 25, marginBottom: 25 },
  authInputHalo:    { backgroundColor: 'rgba(0,0,0,0.8)', borderWidth: 2, borderColor: '#FF0000', borderRadius: 15, padding: 20, color: 'white', textAlign: 'center', fontSize: 24 },
  ...
  keyInput:         { flex: 1, backgroundColor: '#f5f5f5', borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, fontSize: 13, color: '#333', borderWidth: 1, borderColor: '#ddd' },
  saveKeyBtn:       { backgroundColor: '#1D3583', width: 48, height: 48, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
